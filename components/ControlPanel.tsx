...
export const ControlPanel: React.FC<ControlPanelProps> = ({ options, setOptions }) => {
  ...
  return (
    <div className="absolute bottom-0 w-full bg-black/80 border-t border-green-900/50 backdrop-blur-sm p-2 sm:p-4 z-30 transition-all duration-300">
      <div className="max-w-6xl mx-auto flex flex-wrap gap-2 sm:gap-6 justify-center items-center text-green-500 text-[9px] sm:text-xs font-mono">
          ...
          <div className="flex gap-1">
            {(['matrix', 'bw', 'retro', 'color'] as const).map(mode => (
              <button
                ...
                className={`px-1.5 py-0.5 sm:px-2 sm:py-1 border ${options.colorMode === mode ? 'bg-green-500 text-black border-green-500' : 'bg-transparent border-green-800 text-green-700 hover:border-green-500'} text-[9px] sm:text-[10px] uppercase transition-colors`}
              >
                {mode}
              </button>
            ))}
          ...
          <div className="flex gap-1">
            {(Object.keys(DENSITY_MAPS) as Array<keyof typeof DENSITY_MAPS>).map(mode => (
              <button
                ...
                className={`px-1.5 py-0.5 sm:px-2 sm:py-1 border ${options.density === mode ? 'bg-green-500 text-black border-green-500' : 'bg-transparent border-green-800 text-green-700 hover:border-green-500'} text-[9px] sm:text-[10px] uppercase transition-colors`}
              >
                {mode}
              </button>
            ))}
